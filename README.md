...
在Megatron-LM目录下执行：
  ...
  pip install -e .
  ```
> 如需要保存安装megatron_npu的日志，可在pip install命令后面添加参数 `--log <FILE_PATH>`，并对您指定的文件路径`<FILE_PATH>`及其所在目录做好权限管控。
### 2.4 安装其他依赖
  ```shell
...
- megatron_npu的卸载只需执行命令：
  ...
  > 如需要保存卸载megatron_npu的日志，可在pip uninstall命令后面添加参数 `--log <FILE_PATH>`，并对您指定的文件路径`<FILE_PATH>`及其所在目录做好权限管控。
